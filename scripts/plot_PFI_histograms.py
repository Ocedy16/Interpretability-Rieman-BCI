...
if __name__ == "__main__":
    ...
    if dataset == 'pollution' : 
        ...
        for i, subject in enumerate(dataset.subject_list):
            ...
            subject_mean_shap_values = np.mean(shap_values[i],axis=0)
            indices_tries = np.argsort(subject_mean_shap_values)[::-1]
            sensors_tries = np.array(SENSORS)[indices_tries]
            shap_values_triees = subject_mean_shap_values[indices_tries]

            plt.figure(figsize=(12, 6))
            plt.bar(sensors_tries, shap_values_triees)
            plt.title(f'Shapley values of {args.dataset} for the class left_hand - subject {subject}')
            plt.xlabel('Sensors')
            plt.ylabel('Shapley values')
            plt.savefig(f'{OUT_DIR}/histogram_subject_{subject}.pdf')
            plt.show()
